...
macro(add_leatherman_library)
    ...
    if(LEATHERMAN_SHARED)
        add_library(lib${libname} SHARED ${LIBRARY_ARGS})
        target_link_libraries(lib${libname} PRIVATE ${${deps_var}})
    else()
        add_library(lib${libname} STATIC ${LIBRARY_ARGS})
    endif()
    set_target_properties(lib${libname} PROPERTIES COMPILE_FLAGS "${LEATHERMAN_CXX_FLAGS} ${LEATHERMAN_LIBRARY_FLAGS}" VERSION ${PROJECT_VERSION})
    if(LEATHERMAN_INSTALL)
        leatherman_install(lib${libname} EXPORT LeathermanLibraries)
    endif()

    if (EXPORT_HEADER)
        symbol_exports(lib${libname} ${EXPORT_HEADER})
    endif()
    set(${lib_var} "lib${libname}" PARENT_SCOPE)
endmacro()
